...
TABLE:		u_id - a unique user ID (primary key),
		...
		u_type - a user can be a regular user or a guide,
...
CREATE TABLE users (
	...
	CONSTRAINT chk_u_name_len   CHECK (char_length(u_name) >= 5), 
...
CREATE TABLE tours (
	...
	CONSTRAINT duration_positive CHECK (t_duration > 0),
	CONSTRAINT rating_valid CHECK (t_rating >= 0 AND t_rating <= 10)
)
WITH (
  OIDS = FALSE
);
...
TABLE:		t_id - a unique tour ID,
		...
		t_description - general tour's description,
		t_photos - some photos of the tour (submitted by the guide),
		t_languages - the tour is given in these language\s,
		t_date - the tour departs on this date,
		t_time - the tour departs on this time,
		t_price - tour price per person,
		t_vacant - the remaining number of people that can sign up for this tour
*/
...
CREATE TABLE slots (
	...
	t_description	VARCHAR(255),
	t_photos	bytea[],
	t_languages	INTEGER NOT NULL, -- how to apply more than one language?
	t_date		date    NOT NULL,
	t_time		TIME WITHOUT TIME ZONE NOT NULL,
	t_price		NUMERIC NOT NULL,
	t_vacant 	INTEGER NOT NULL
	CONSTRAINT price_non_negative CHECK (t_price >= 0),
	CONSTRAINT vacant_non_negative CHECK (t_vacant >= 0)
)
WITH (
  OIDS = FALSE
);
...
TABLE:		u_id - a unique user ID of the requesting user
		...
		t_num_participants - number of participants that are signed to the tour under the current user 
...
CREATE TABLE requests (
	...
	t_id		   INTEGER NOT NULL REFERENCES tours(t_id) ON DELETE RESTRICT,
	ts_id		   INTEGER NOT NULL REFERENCES slots(ts_id) ON DELETE RESTRICT,
	t_num_participants INTEGER NOT NULL,
	CONSTRAINT participants_non_negative CHECK (t_num_participants >= 0),
...
CREATE TABLE rate2guide (
	...
	CONSTRAINT rating_valid CHECK (g_rating >= 0 AND g_rating <= 10),
	PRIMARY KEY (u_id, t_id, g_id)
)
WITH (
  OIDS = FALSE
);
