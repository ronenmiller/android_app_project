...
CREATE OR REPLACE FUNCTION validate_credentials(_u_var VARCHAR(80), _u_pass VARCHAR(80))
	RETURNS BOOLEAN AS 
$$
DECLARE
	temp users;
BEGIN
	...
	INTO temp
	FROM users 
	WHERE ((users.u_name = _u_name OR users.email = _u_var) AND users.u_pass = _u_pass); 
